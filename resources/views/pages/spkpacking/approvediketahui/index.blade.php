<x-default-layout>
    @section('title', 'Approve Diketahui - SPK Packing Member')

    <div class="card mt-5">
        <div class="card-header">
            <h3 class="card-title">Halaman Approve Diketahui</h3>
        </div>
                    ...
                    <select id="tanggal_filter" class="form-select">
                        ...
                        @foreach($tanggalProsesList as $tgl)
                            @php
                                ...
                                $isApproved = !is_null($tgl->approved_diketahui_at);
                            @endphp
                            <option 
                                value="{{ $tgl->tanggal_proses }}" 
                                data-approved="{{ $tgl->approved_diketahui_at }}"
    ...
    <script>
        ...
        $('#tanggal_filter').on('change', function () {
            ...
            $.get("{{ route('spkpacking.approvediketahui.getbytanggal') }}", { tanggal_proses: tanggal }, function (data) {
                if (!data.length) {
        ...
        $('#save-all').on('click', function () {
            ...
            $.post("{{ route('spkpacking.approvediketahui.bulkupdate') }}", {
                _token: '{{ csrf_token() }}',
                details: details
            }, function (res) {
                Swal.fire({
        ...
        $('#approve-form').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('spkpacking.approvediketahui.approve') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
