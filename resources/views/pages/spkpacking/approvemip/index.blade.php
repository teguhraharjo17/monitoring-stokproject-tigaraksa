<x-default-layout>
    @section('title', 'Approve MIP - SPK Packing Member')

    <div class="card mt-5">
        <div class="card-header">
            <h3 class="card-title">Halaman Approve MIP</h3>
        </div>
                    ...
                    <select id="tanggal_filter" class="form-select">
                        ...
                        @foreach($tanggalProsesList as $tgl)
                            @php
                                ...
                                $isApproved = !is_null($tgl->approved_mip_at);
                            @endphp
                            <option 
                                value="{{ $tgl->tanggal_proses }}" 
                                data-approved="{{ $tgl->approved_mip_at }}"
    ...
    <script>
        ...
        $('#tanggal_filter').on('change', function () {
            ...
            $.get("{{ route('spkpacking.approvemip.getbytanggal') }}", { tanggal_proses: tanggal }, function (data) {
                if (!data.length) {
        ...
        $('#save-all').on('click', function () {
            ...
            $.post("{{ route('spkpacking.approvemip.bulkupdate') }}", {
                _token: '{{ csrf_token() }}',
                details: details
            }, function (res) {
                Swal.fire({
        ...
        $('#approve-form').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('spkpacking.approvemip.approve') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
