<x-default-layout>
    @section('title', 'Approve FG - SPK Packing Member')

    <div class="card mt-5">
        <div class="card-header">
            <h3 class="card-title">Halaman Approve Finish Good</h3>
        </div>
                    ...
                    <select id="tanggal_filter" class="form-select">
                        ...
                        @foreach($tanggalProsesList as $tgl)
                            @php
                                ...
                                $isApproved = !is_null($tgl->approved_fg_at);
                            @endphp
                            <option 
                                value="{{ $tgl->tanggal_proses }}" 
                                data-approved="{{ $tgl->approved_fg_at }}"
    ...
    <script>
        ...
        $('#tanggal_filter').on('change', function () {
            ...
            $.get("{{ route('spkpacking.approvefg.getbytanggal') }}", { tanggal_proses: tanggal }, function (data) {
                if (!data.length) {
        ...
        $('#save-all').on('click', function () {
            ...
            $.post("{{ route('spkpacking.approvefg.bulkupdate') }}", {
                _token: '{{ csrf_token() }}',
                details: details
            }, function (res) {
                Swal.fire({
        ...
        $('#approve-form').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('spkpacking.approvefg.approve') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
