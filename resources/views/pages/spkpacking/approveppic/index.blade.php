<x-default-layout>
    @section('title', 'Approve PPIC - SPK Packing Member')

    <div class="card mt-5">
        <div class="card-header">
            <h3 class="card-title">Halaman Approve PPIC</h3>
        </div>
                    ...
                    <select id="tanggal_filter" class="form-select">
                        ...
                        @foreach($tanggalProsesList as $tgl)
                            @php
                                ...
                                $isApproved = !is_null($tgl->approved_ppic_at);
                            @endphp
                            <option 
                                value="{{ $tgl->tanggal_proses }}" 
                                data-approved="{{ $tgl->approved_ppic_at }}"
                ...
                <div class="modal-content">
                    <form id="approve-form" enctype="multipart/form-data">
                        ...
                        <div class="modal-header">
                            <h5 class="modal-title" id="approveModalLabel">Upload Tanda Tangan PPIC</h5>
    ...
    <script>
        ...
        $('#tanggal_filter').on('change', function () {
            ...
            $.get("{{ route('spkpacking.approveppic.getbytanggal') }}", { tanggal_proses: tanggal }, function (data) {
                if (!data.length) {
                            ...
                            <tr data-id="${detail.id}">
                                ...
                                <td>
                                    <select class="form-select form-select-sm select2-partnumber" style="width: 100%;" data-row="${row}">
                                        ${data.part_number ? `<option selected value="${data.part_number}">${data.part_number}</option>` : ''}
                                    </select>
                                </td>
        ...
        $('#save-all').on('click', function () {
            ...
            $.post("{{ route('spkpacking.approveppic.bulkupdate') }}", {
                _token: '{{ csrf_token() }}',
                details: details
            }, function (res) {
                Swal.fire({
        ...
        $('#approve-form').on('submit', function (e) {
            ...
            $.ajax({
                url: "{{ route('spkpacking.approveppic.approve') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
